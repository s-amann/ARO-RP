...
exist and are correctly configured to automicatically mitigate non ready worker nodes.

There are two flags which control the operations perfromed by the controller:
...
- When set to false, the controller will attempt to remove the aro-machinehealthcheck CR from the cluster.
  This should effectivly disable the MHC we deploy and prevent the automatic reconsilation of nodes.
