...
* pkg: RP source code:
  ...
  * pkg/operator/controllers: A list of controllers instantiated by the operator
    ...
    * machinehealthcheck: Ensures the MachineHealthCheck resource is running as configured so that at most one worker node at a time is automatcially
      reconciled when not ready for at least 5 minutes.
        ...
        * When `aro.machinehealth.true` is `"false"` and `aro.machinehealthcheck.managed` is `"false"` the CR will be removed from the cluster.
